<?php
 ...
 if($_SERVER['REQUEST_METHOD'] == 'POST'){
     ...
     $sql = "SELECT COUNT(aten_id) FROM atendente WHERE aten_nome = '$nome'
     AND aten_senha = '$senha'";
     ...
     if($cont == 1){
         $sql = "SELECT * FROM atendente WHERE aten_nome = '$nome' 
         AND aten_senha = '$senha'"; 



    #Verificação se quem fez cadastro foi usuario ou atendente
        $retorno = mysqli_query($link,$sql);
        while($tbl= mysqli_fetch_array($retorno))
        {
        $_SESSION['nomeatendente'] = $tbl[1];
        $_SESSION['idaten'] = $tbl[0];
         #DIRECIONA USUARIO PARA O ADM
         echo"<script>window.location.href='atendentehome.php';</script>";
        }
...
<!----------------------------- Form box ----------------------------------->    
    <div class="form-box">
        ...
        <!------------------- login form -------------------------->
        <form action="atendentelogin.php" class="login-container" id="login" method="post"> <!-- Form é necessario para informar que o elemento representa um formulario que sera preenchido pelo php -->
        <div>
            <div class="top">

 ...
 if($_SERVER['REQUEST_METHOD'] == 'POST')
{
    ...
    $sql = "SELECT COUNT(aten_cpf) FROM atendente WHERE aten_cpf = '$cpf'";
    ...
    if($cont == 1)
    {
        echo"<script>window.alert('ATENDENTE JÁ EXISTE');</script>";
    }
    else
    {
        $sql = "INSERT INTO atendente (aten_nome, aten_senha, aten_email, aten_cpf, aten_horainicio, aten_horafim, aten_datanasc, aten_telefone, aten_cidade, aten_endereco, aten_numero, aten_ativo) 
        VALUES('$nome','$senha','$email','$cpf','$horainicio','$horafim','$datanasc','$telefoneNumerico','$cidade','$endereco','$numero','s')"; # 'n' representa usuario não ativo
                                                                                                        # posso colocar 's' para usuario ativo
        mysqli_query($link, $sql);
        #cadastra cliente e joga mensagem na tela e direciona para lista usuario
        echo"<script>window.alert('ATENDENTE CADASTRADO');</script>";
        echo"<script>window.location.href='contrataAtendente.php';</script>";
...
<head>
    ...
    <title>Contratar Atendente</title>
</head>
...
<div class="form_box">
        <form action="contrataAtendente.php" method="post">
            <fieldset>
                <legend><b>Contrata Atendente</b></legend>
                <br>
...
<!-- FIM MEIO -->
    ...
    <!-- IMPEDE CPF DE ADICIONAR LETRAS E ADICIONA (.) E (-)  -->
